...
j=6: coin=1 -> dp[6]=min(12, dp[5]+1)=2
     coin=2 -> dp[6]=min(2,  dp[4]+1)=2
     coin=5 -> dp[6]=min(2,  dp[1]+1)=2
j=7  coin=1 -> dp[7]=min(12, dp[6]+1)=3
     coin=2 -> dp[7]=min(3,  dp[5]+1)=2
     coin=5 -> dp[7]=min(2,  dp[2]+1)=2
j=8  coin=1 -> dp[8]=min(12, dp[7]+1)=3
     coin=2 -> dp[8]=min(3,  dp[6]+1)=3
     coin=5 -> dp[8]=min(3,  dp[3]+1)=3
j=9  coin=1 -> dp[9]=min(12, dp[8]+1)=4
     coin=2 -> dp[9]=min(4, dp[7]+1)=3
     coin=5 -> dp[9]=min(3, dp[4]+1)=3
j=10 coin=1 -> dp[10]=min(12, dp[9]+1)=4
     coin=2 -> dp[10]=min(4, dp[7]+1)=3
     coin=5 -> dp[10]=min(3, dp[5]+1)=2
j=11 coin=1 -> dp[11]=min(12, dp[10]+1)=3
